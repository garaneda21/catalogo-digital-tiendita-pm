    ...
    ->add(match ($variant) {
        'outline' => match ($accent) {
            true => [
                ...
                'hover:text-zinc-800 dark:hover:text-white dark:hover:bg-white hover:bg-zinc-800/5 ',
        ...
        default => match ($accent) {
            true => [
                ...
                'hover:text-zinc-800 hover:bg-white',
            ],
            false => [
...
<flux:button-or-link :attributes="$attributes->class($classes)" data-flux-navlist-item>
    ...
    <?php if ($slot->isNotEmpty()): ?>
        <div class="flex-1 text-md font-medium leading-none whitespace-nowrap [[data-nav-footer]_&]:hidden [[data-nav-sidebar]_[data-nav-footer]_&]:block" data-content>{{ $slot }}</div>
    <?php endif; ?>
